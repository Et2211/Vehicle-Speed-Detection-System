...
def firstCrop(img, predictions):
    ...
    cv2.rectangle(img, (xtop, ytop), (xbottom, ybottom), (0, 0xFF, 0),
                  3)
    return firstCrop
...
def secondCrop(img):
    ...
    (ret, thresh) = cv2.threshold(gray, 127, 0xFF, 0)
    ...
    try:
        ...
        if counter % 15 == 0:
            try:
                ...
                secondCropImg = cv2.cvtColor(secondCropImg,
                        cv2.COLOR_BGR2GRAY)

                if not os.path.exists(dirname + '/results'):
                    os.mkdir(dirname + '/results')

                cv2.imwrite(dirname + '/results/result' + str(plateNum)
                            + '.jpg', secondCropImg)
                rtnJSON = '{"source": "./results/result' \
                    + str(plateNum) \
                    + '.jpg", "camera": "1" , "time": "' \
